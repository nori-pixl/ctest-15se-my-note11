...
def v_thread(cid, tid):
    ...
    thread = thread_data if isinstance(thread_data, list) and len(thread_data) > 0 else thread_data
    return render_template(
        'board.html', 
        v='thread', 
        cid=cid, 
        tid=tid, 
        tname=thread.get('title', '不明') if isinstance(thread, dict) else "不明", 
        items=res.get("posts", []), 
        login_user="名無しさん", 
        count=get_image_upload_count()
    )
...
def post(cid, tid):
    if cid != 1: 
        return redirect('/')
    name, message = request.form.get('name', '名無しさん'), request.form.get('b', '')
    try: 
...
def post_chunk(cid, tid):
    ...
    if upload_count >= 5: 
        return "本日の画像アップロード上限（5回）に達しました。", 400

    chunk_file = request.files.get('image_chunk')
    if not chunk_file: 
        return "No chunk file", 400
    ...
    try:
        api_res = requests.post(
            f"{TERMUX_API_BASE}/api/posts_chunk", 
            data=data, 
            files={'image_chunk': (chunk_file.filename, chunk_file.stream, chunk_file.content_type)}, 
            timeout=30
        ).json()
    except Exception as e:
        return f"タブレットへの通信エラー: {str(e)}", 502

    if not api_res.get("success"): 
        return "タブレット側での保存に失敗しました。", 500
    ...
    if str(request.form.get('chunk_index')) == "9" and api_res.get("complete"):
        ...
        resp.set_cookie('img_upload_count', str(upload_count + 1), expires=expires)
    return resp
